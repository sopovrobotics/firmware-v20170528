<?php
	...
	$settings = array();
	$settings['ipaddress_robot'] = array(
		'caption' => 'IP Address (robot)'
	);
	$settings['secret_robot'] = array(
		'caption' => 'Secret (robot)'
	);
...
<html>
	...
	<div class="container">
				...
				while($row = $stmt->fetch()){
					...
					echo '
						<div class="form-group row">
							<div class="col-xs-12">
								<label for="'.$name.'">'.$settings[$name]['caption'].'</label>
								<input name="'.$name.'" id="'.$name.'" style="font-size: medium;" value="'.$value.'" class="form-control">
							</div>
						</div>
					';
				}
		...
		?>
		<button name="update_settings" class="btn btn-danger">Update</button>
		</form>
		
		<button id="check_connection" class="btn btn-success">Check Conenction To Robot (<?php echo $settings['ipaddress_robot']['value']; ?>)</button> 
		...
		<script>
			var ipaddress = "<?php echo $settings['ipaddress_robot']['value']; ?>";
			sopovrobotics.secret = "<?php echo $settings['secret_robot']['value']; ?>";
			var settings_cache = {};
